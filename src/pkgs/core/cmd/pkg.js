    ...
    enable: (command, msg) => {
        ...
        if (command in bot.pkgMap) {
            ...
            if (command in pkgs) {
                ...
                bot.load(pkgs[command])
                msg.channel.send(`${pkgs[command]} 패키지가 활성화 되었습니다.`)
                data.guild.save(msg.guild.id, bot.setting)
            } else {
                msg.channel.send(`${pkgs[command]} 패키지를 찾을 수 없습니다.`)
            }
        }
        return true
    },
